...
In Kenya, avocado production spans diverse agroecological zones. The two study regions, Embu and Elgeyo Marakwet, illustrate this variability. Embu lies in the warm, humid mid‑elevation foothills of Mt. Kenya (≈1,200–1,800 m), characterised by deep volcanic Nitisols, high annual rainfall and intensive smallholder orchards often intercropped with other crops (Jaetzold et al., 2012; @jaetzold2012farm; Embu County CIDP, 2018; @embuCIDP2018). In contrast, Elgeyo Marakwet occupies cooler highland conditions along the Rift Valley escarpment (≈1,800–2,600 m), with steeper terrain, higher irradiance, more variable rainfall, and wider orchard spacing (Jaetzold et al., 2012; @jaetzold2012farm; Elgeyo Marakwet County CIDP, 2018; @elgeyoCIDP2018). These environmental and management contrasts influence canopy water dynamics, vegetative growth rates, and phenological timing, making them ideal for evaluating the robustness of remote‑sensing‑based phenology monitoring across heterogeneous smallholder systems.

Remote sensing offers a promising alternative. Sentinel‑2 and Landsat 8/9 deliver 10–30 m spatial resolution and frequent revisit intervals, producing dense time series capable of capturing canopy dynamics and phenological transitions in perennial crops [@bolton2021continental; @zhang2003monitoring]. Recent work has demonstrated that Sentinel‑2 time‑series vegetation indices can reliably track avocado phenology across flowering, vegetative growth, fruit development, and maturity stages [@Rahman2022]. However, applications in Kenyan smallholder systems remain limited due to heterogeneous canopies, intercropping and variable management practices [@jaetzold2012farm]. Similar challenges have been documented in other African smallholder fruit‑tree systems, where Sentinel‑2 phenological metrics have nonetheless shown strong potential for crop discrimination and monitoring [@Chabalala2023].

Embu is an established avocado‑producing region with strong market linkages, while Elgeyo Marakwet is an emerging production zone with expanding commercial interest (Elgeyo Marakwet County CIDP, 2018; @elgeyoCIDP2018).
...
Sentinel‑2 Multispectral Instrument (MSI) and Landsat 8/9 Operational Land Imager (OLI) surface reflectance products were accessed via Google Earth Engine, following operational EO data methodologies for agricultural monitoring [@Suarez2020]. The analysis used the spectral bands required for vegetation index computation, including the visible (Blue, Green, Red), near‑infrared (NIR), red‑edge (RE1–RE4), and shortwave infrared (SWIR1–SWIR2) bands from Sentinel‑2 MSI, and the corresponding Blue, Green, Red, NIR, and SWIR bands from Landsat OLI. Cloud masking was performed using Sentinel‑2 Cloud Probability and Landsat CFMask, following established best practices for multisensor phenology monitoring [@roy2016landsat; @claverie2018harmonized].
...
Vegetation indices (NDVI, EVI, NDWI, NDRE) were derived in GEE for each orchard polygon. These indices capture complementary physiological processes associated with avocado canopy development. NDVI quantifies canopy greenness and photosynthetic activity, increasing during flowering and early vegetative growth but saturating in dense evergreen canopies (Huete et al., 2002; @huete2002modis). EVI enhances sensitivity to canopy structure and leaf area, reducing saturation effects and capturing structural changes during vegetative flush (Huete et al., 2002; @huete2002modis). NDWI measures canopy water content and is strongly responsive to the substantial increase in leaf and fruit water during early fruit expansion (Gao, 1996; @gao1996ndwi). NDRE, derived from the red‑edge region, is sensitive to chlorophyll concentration and early‑season pigment dynamics, providing additional discrimination during flowering and initial vegetative growth (Gitelson et al., 2005; @gitelson2005remote). Together, these indices provide a physiologically meaningful representation of avocado phenology and have been shown to track seasonal transitions in commercial orchards (Rahman et al., 2022; @Rahman2022).
...
Classification and Regression Trees (CART) is a supervised machine‑learning classification technique that learns decision rules from labelled training data to predict categorical outcomes. Three CART models were trained:
